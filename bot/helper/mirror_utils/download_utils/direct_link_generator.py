# Modern direct link generator (full port from modern reference)
# Adapted for fixvid bot (config_dict, LOGGER, lxml-only HubCloud)
...
from urllib.parse import parse_qs, urlparse, quote, unquote, urljoin
...
from bot.helper.ext_utils.links_utils import (
    is_sharer_link as is_share_link,
    is_gdrive_link,
)
...
# Compatibility shims
# ------------------------------------------------------------
PASSWORD_ERROR_MESSAGE = HelpString.PASSWORD_ERROR_MESSAGE


class _ConfigShim:
    """Maps Config.<NAME> → config_dict.get('<NAME>', '')"""
...
debrid_link_supported_sites = [
    "1fichier.com", "anonfiles.com", "bayfiles.com",
    "clicknupload.link", "clicknupload.org", "clicknupload.co",
    "clicknupload.cc", "clicknupload.download", "clicknupload.club",
    "dailyuploads.net", "ddl.to", "ddownload.com", "ddownload.link",
    "drop.download", "dropbox.com", "dropboxusercontent.com",
    "easyupload.io", "emload.com", "file.al", "fileaxa.com",
    "filecat.net", "filedot.to", "filedot.xyz", "filextras.com",
    "filer.net", "filespace.com", "filestore.me", "gigapeta.com",
    "gofile.io", "hexupload.net", "hitfile.net", "hulkshare.com",
    "isra.cloud", "katfile.com", "kshared.com", "mediafire.com",
    "mega.nz", "mega.co.nz", "mexashare.com", "mixdrop.co",
    "mixdrop.to", "mixdrop.sx", "mixdrop.club", "modsbase.com",
    "nelion.me", "pixeldrain.com", "prefiles.com", "racaty.net",
    "rapidgator.net", "rapidgator.asia", "rg.to", "scribd.com",
    "send.cm", "sharemods.com", "silkfiles.com", "soundcloud.com",
    "streamtape.com", "terabox.com", "teraboxapp.com", "tezfiles.com",
    "turb.cc", "turb.to", "turbobit.net", "turbobit.cc",
    "turbobit.pw", "turbobit.online", "turbobit.ru", "turbobit.live",
    "trubobit.com", "turboblt.co", "uloz.to", "ulozto.net",
    "ulozto.sk", "ulozto.cz", "upload.ee", "uploadhaven.com",
    "up-4ever.com", "up-4ever.net", "uptobox.com", "uptobox.fr",
    "uptobox.eu", "uptobox.link", "uptostream.com", "uptostream.fr",
    "uptostream.eu", "uptostream.link", "upvid.pro", "upvid.live",
    "upvid.host", "upvid.biz", "upvid.cloud", "uqload.com",
    "uqload.co", "uqload.io", "userload.co", "usersdrive.com",
    "vidoza.net", "voe.sx", "voe-unblock.com", "voeunblock1.com",
    "voeunblock2.com", "voeunblock3.com", "voeunbl0ck.com",
    "voeunblck.com", "voeunblk.com", "voe-un-block.com",
    "voeun-block.net", "workupload.com", "world-bytez.com",
    "worldbytez.com", "world-files.com", "wupfile.com", "zippyshare.com",
]
...
def direct_link_generator(link):
    ...
    if not domain:
        ...
        for x in [
            ...
            "dood.video", "dooood.com", "dood.yt", "doods.yt",
            "dood.stream", "doods.pro", "ds2play.com", "d0o0d.com",
        ...
        for x in [
            ...
            "streamtape.to", "streamtape.net", "streamta.pe",
            "streamtape.xyz",
        ]
    ):
        ...
        for x in [
            ...
            "1024tera.com", "terabox.app", "gibibox.com",
        ...
        for x in [
            ...
            "filelions.to", "mycloudz.cc", "cabecabean.lol",
            "filelions.online", "embedwish.com", "kitabmarkaz.xyz",
            "wishfast.top", "streamwish.to", "kissmovies.net",
        ]
    ):
    ...
    # -------- HUB FAMILY --------
        ...
        x in domain
        for x in ["linkbox.to", "lbx.to", "teltobx.net", "telbx.net"]
    ):
        return linkBox(link)
        ...
        x in domain
        for x in [
            "anonfiles.com", "zippyshare.com", "letsupload.io",
            "hotfile.io", "bayfiles.com", "megaupload.nz",
            "letsupload.cc", "filechan.org", "myfile.is",
            "vshare.is", "rapidshare.nu", "lolabits.se",
...
"""---- HubCloud & GDFlix Bypass (lxml-only) ----"""
...
def _parse_html(text):
    """lxml-only HTML parser (no bs4 dependency)."""
    if not text:
        return None
    try:
        return HTML(text)
...
def gdflix_extract_pack_links(html, base):
    out = []
    matches = re.findall(r'href="(/file/[A-Za-z0-9]+)"', html or "")
    for m in matches:
        full = urljoin(base, m)
        if full not in out:
            out.append(full)
...
"""__________________________________________________"""
"""---- All other site bypass functions ----"""
...
def debrid_link(url):
    ...
    if resp.get("success") is not True:
        raise DirectDownloadLinkException(
            f"ERROR: {resp.get('error')} & ERROR ID: {resp.get('error_id')}"
        )
...
def osdn(url):
    with create_scraper() as session:
        try:
            ...
            raise DirectDownloadLinkException(f"ERROR: {e.__class__.__name__}") from e
        # NOTE: original had typo `xapth` — fixed to `xpath`
...
def onedrive(link):
    """Onedrive direct link generator (by https://github.com/junedkh)"""
    with create_scraper() as session:
        try:
        ...
        try:
            resp = session.get(
                ...
                f"/items/{folder_id}?$select=id,@content.downloadUrl&ump=1&authKey={authkey}",
                headers=headers,
                data=data,
            ).json()
        except Exception as e:
...
def fichier(link):
    """1Fichier direct link generator (based on https://github.com/Maujar)"""
...
def sharer_scraper(url):
    ...
    try:
        ...
        header = {
            ...
            "AppleWebKit/534.10 (KHTML, like Gecko) Chrome/7.0.548.0 Safari/534.10"
        }
...
def gofile(url):
    """Modern GoFile (SHA256 X-Website-Token + token revoke retry)."""
    try:
        if "::" in url:
            _password = url.split("::")[-1]
    ...
    def __fetch_links(session, _id, folderPath="", retry=True):
        ...
        if _json.get("status") in (
            "error-unauth", "error-forbidden", "error-tokenInvalid"
        ):
            global gofile_token_cache
            gofile_token_cache = None
            if retry:
    ...
    def __writeContents(html_text, folderPath=""):
        ...
        for file in files:
            if not (link := __getFile_link(file["file_id"])):
                continue
            # Note: original used file["filename"] which was a typo; fixed to file_name
...
def filelions_and_streamwish(url):
    ...
    if any(
        x in hostname
        for x in [
            "filelions.co", "filelions.live", "filelions.to",
            "filelions.site", "cabecabean.lol", "filelions.online",
            "mycloudz.cc",
        ]
    ):
        ...
        x in hostname
        for x in [
            "embedwish.com", "kissmovies.net", "kitabmarkaz.xyz",
...
def qiwi(url):
    """qiwi.gg link generator (based on https://github.com/aenulrofik)"""
    with Session() as session:
        file_id = url.split("/")[-1]
        try:
...
def mp4upload(url):
    with Session() as session:
        try:
            ...
            data = {
                inp.get("name"): (inp.get("value") or "").replace(" ", "")
                for inp in inputs
            }
            if not data:
                raise DirectDownloadLinkException("ERROR: File Not Found!")
            ...
            return direct_link, f"Referer: https://www.mp4upload.com/"
        except DirectDownloadLinkException:
            raise
        except Exception:
            raise DirectDownloadLinkException("ERROR: File Not Found!")
...
def berkasdrive(url):
    """berkasdrive.com link generator (by https://github.com/aenulrofik)"""
    with Session() as session:
        try:
...
def swisstransfer(link):
    ...
    if len(files) == 1:
        ...
        download_url = (
            f"https://{download_host}/api/download/{transfer_id}/{file_uuid}?token={token}"
        )
